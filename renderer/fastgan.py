...
class Encoder(nn.Module):
    def __init__(self, n_emojis: int = 4099, embed_dim: int = 28, out_dim: int = 128):
        super().__init__()
        self.embed = nn.Embedding(n_emojis, embed_dim)

        self.linear = nn.Sequential(
            self.block(embed_dim + 4, out_dim // 2),
...
class Discriminator(nn.Module):
    ...
    def forward(self, imgs, label, part=None):
        if type(imgs) is not list:
            imgs = [
                F.interpolate(imgs, size=self.im_size),
                F.interpolate(imgs, size=128),
            ]
