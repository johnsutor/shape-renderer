...
class Encoder(nn.Module):
    def __init__(self, 
        ...
        self.net = nn.ModuleList([
            nn.Conv2d(in_chan, first_chan, 5, 1, 2),
            nn.LeakyReLU(0.1)
        ])
...
class VQVAE(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        enc_kwargs = {key.split("encoder_")[1]: value for key, value in kwargs.items() if key.startswith("encoder")}
        dec_kwargs = {key.split("decoder_")[1]: value for key, value in kwargs.items() if key.startswith("decoder")}
